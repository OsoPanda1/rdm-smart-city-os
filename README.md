...
## 4.1) Modelo de Dominio Canónico

Este es el contrato institucional del sistema para evitar fragmentación conceptual entre equipos, servicios y generaciones de código.

### Entidades raíz (Aggregate Roots)

- `TERRITORY`: unidad soberana operativa (ej. RDM).
- `TERRITORY_SESSION`: sesión contextual de interacción territorio-visitante.
- `DECISION_CASE`: caso de decisión generado por ISABELLA.
- `PAYMENT_ORDER`: orden económica trazable de Cattleya Pay.
- `GEO_FENCE`: regla geoespacial de control territorial.

### Tipos de eventos canónicos

- Territorio: `TERRITORY_REGISTERED`, `TERRITORY_ACTIVATED`, `TERRITORY_DEGRADED`, `TERRITORY_RECOVERED`, `TERRITORY_SUSPENDED`.
- Sesión: `SESSION_STARTED`, `SESSION_INTERACTION_RECORDED`, `SESSION_AT_RISK_DETECTED`, `SESSION_RETAINED`, `SESSION_CLOSED`.
- Decisión: `DECISION_EVALUATED`, `DECISION_ACTION_DISPATCHED`, `DECISION_REVIEWED`.
- Economía: `PAYMENT_INITIATED`, `PAYMENT_SESSION_CREATED`, `PAYMENT_AUTHORIZED`, `PAYMENT_CAPTURED`, `PAYMENT_SETTLED`, `PAYMENT_FAILED`, `PAYMENT_REFUNDED`.
- Geo: `GEOFENCE_DEFINED`, `GEOFENCE_ENFORCED`, `GEOFENCE_TRIGGERED`, `GEOFENCE_ARCHIVED`.
...
### Reglas de transición (state machine)

Las transiciones se definen y validan técnicamente en:

- `core/tamv-os-kernel/src/domain/canonical-domain.ts` (`resolveNextState`, `getTransitionRules`).
- `core/tamv-os-kernel/src/event-store.ts` (validación opcional de progresión canónica al persistir eventos).
- `economy/cattleya-pay/src/payment.service.ts` (aplicación explícita de transición canónica para pagos).

> Doctrina TAMV Online: ningún servicio debe introducir estados o eventos fuera del modelo canónico sin versionado explícito del contrato de dominio.
...
## Pipeline recomendado de despliegue productivo

1. Aplicar migraciones DB (incluye `tamv_event_store` y `cattleya_payment_ledger`):
   ```bash
   supabase db push
   ```
2. Desplegar gateway y kernel en Kubernetes (`infra/kubernetes/*`).
...
- Organización de referencia: **TAMV ONLINE NETWORK (México)**.
- CEO Fundador: **EDWIN OSWALDO CASTILLO TREJO (ANUBIS VILLASEÑOR)**.
- Referencia de identidad pública usada para alineación documental: perfil GitHub `OsoPanda1 / TAMV ONLINE MEXICO`.
