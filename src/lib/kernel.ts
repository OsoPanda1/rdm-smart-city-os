...
function narrativeByDecision(decision?: IsabellaDecision) {
  ...
  if (decision.retentionIntent === "SAFE_EXIT") {
    return "Te acompaño con una salida segura: caminemos por rutas visibles y con paradas culturales cortas para cerrar tu visita en calma.";
  }

  if (decision.retentionIntent === "UPSELL") {
    return "Antes de partir, aprovecha una última oportunidad local: una parada de alto valor cultural y gastronómico a pocos minutos.";
  }

  return "Modo descubrimiento activo: hay experiencias cercanas para explorar sin alejarte de tu zona segura.";
}
